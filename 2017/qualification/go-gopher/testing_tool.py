...
def random_output(x, y) :
    ...
    random_idx = np.random.choice(len(neigh), 1)[0]
    return neigh[random_idx]
...
for i in range(NUM_TEST_CASES):
    ...
    for _ in range(N):
        ...
        if (x <= 1) or (x > 999) or (y <= 1) or (y > 999):
        ...
        # print("TOO_SMALL", file=p.stdin)
        user_input_history.append((x,y))
        new_x, new_y = random_output(x, y)
        if not test_case_passed :
            print("{} {}".format(x, y), file=p.stdin)
        if test_case_passed :
            print("{} {}".format(0, 0), file=p.stdin)
    if not test_case_passed:
        print("{} {}".format(-1, -1), file=p.stdin)
